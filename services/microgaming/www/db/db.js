...
const startDB = (app, db_type) => {
  switch (db_type) {
    case "mysql":
      console.log(
        `Environment : ${process.env.NODE_ENV} Database : ${process.env.DATABASE_TYPE}`
      );
      ...
      try {
        dataAPI.authenticate().then(() => {
          console.log(
            `Database Connection open Success : ${JSON.stringify(
              dbConfig.host
            )}`
          );
      ...
      try {
        ...
        mongoose.connection.on("open", async function (err) {
          if (err) {
            ...
            const redisConnection = await redisLib.connect(appConfig.redis_url);
